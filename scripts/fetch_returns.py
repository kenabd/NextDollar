...
def main() -> None:
    ...
    assets = [
        ...
        row("IWM", "Russell 2000 ETF", small, "Proxy: US small-cap stocks from Damodaran table."),
        row("DIA", "Dow Jones ETF", stock, "Proxy: US large-cap stocks (S&P 500 total return proxy)."),
        row("VUG", "US Growth ETF", small, "Proxy: US small-cap/growth-heavy long-horizon return proxy."),
        row("VTV", "US Value ETF", stock, "Proxy: US broad large-cap total return series."),
        row("XLK", "Technology Sector ETF", small, "Proxy: US growth-heavy equity return series."),
        row("BND", "US Aggregate Bond ETF", bond, "Proxy: US Treasury bond return series."),
        row("TLT", "Long-Term Treasury ETF", bond, "Proxy: US Treasury bond return series."),
